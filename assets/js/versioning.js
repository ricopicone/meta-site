...
document.querySelectorAll("section.real-section.level1").forEach(function(element) {
	...
	// Identify the pdf file, which has the format "x-y.pdf", whereas data-0-sec has the format "x.y" (x and y can be 1 or 2 digits)
	var pdf_filename = section_number.replace(/\./g, "-") + ".pdf";

	// Check if the pdf file exists
	var pdf_file_exists = false;
	var http = new XMLHttpRequest();
	http.open('HEAD', "/assets/split/partial/" + pdf_filename, false);
	http.send();
	if (http.status != 404) {
